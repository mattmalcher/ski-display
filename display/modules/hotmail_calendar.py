"""Hotmail / Outlook Calendar module — shows upcoming events from an iCal URL.

No OAuth required. Get the ICS (iCal) link from Outlook.com:
    ...
    4. Under "Publish a calendar", choose your calendar and set permissions
    5. Click Publish — copy the ICS link shown

Add to config.json:
    {
        "name": "hotmail_calendar",
        "enabled": true,
        "ical_url": "https://outlook.live.com/owa/calendar/.../reachcalendar.ics",
        "days_ahead": 7,
        "fetch_interval": 900
    }
...
class Module(DisplayModule):
    def start(self):
        ...
        if not _DEPS_OK:
            logger.error(
                'hotmail_calendar: icalendar and/or recurring-ical-events not installed '
                '(pip install icalendar recurring-ical-events)'
            )
            return

        if not self._ical_url:
            logger.warning('hotmail_calendar: no ical_url configured, module inactive')
            return
    ...
    def _fetch_loop(self):
        while not self._stop.is_set():
            try:
                ...
                logger.exception('hotmail_calendar: fetch error')
    ...
    def _fetch(self):
        ...
        for event in sorted(occurrences, key=lambda e: _sort_key(e)):
            try:
                ...
                logger.exception('hotmail_calendar: error processing event %r', event.get('SUMMARY'))
